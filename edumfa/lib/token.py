    ...
    if tokeninfo:
        ...
        if type(tokeninfo) is not dict:
            raise ParameterError(
                "Parameter 'info' must be a string representation of a json object (key-value pairs) or a python dict. but is of type {0!r}",
                type(tokeninfo),
            )
...
def create_challenge_without_token(reply_dict, options=None):
    ...
    r_chal, message, transaction_id, challenge_info = (
        WebAuthnTokenClass.create_usernameless_challenge(options)
    )
