...
myApp.controller("policyDetailsController", ["$scope", "$stateParams",
    ...
    ConfigFactory.getEduMfaNodes(function (data) {
        $scope.edumfanodes = [];
        angular.forEach(data.result.value, function(value, key){
            $scope.edumfanodes.push({name: value, ticked: false});
        });
        $scope.edumfaNodesLoaded = true;
        check_all_loaded();
    });
    ...
    $scope.createPolicy = function () {
        ...
        // get Edumfanodes
        angular.forEach($scope.selectededumfaNodes, function(value, key){
            edumfanodes.push(value.name);
            $scope.params.edumfanode = edumfanodes;
        });
        // get resolvers
    ...
    $scope.presetEditValues2 = function(policy) {
            ...
            angular.forEach($scope.edumfanodes, function(value, key){
                if (policy.edumfanode.indexOf(value.name) > -1 ) {
                    $scope.edumfanodes[key].ticked = true;
                }
            });
...
myApp.controller("tokenConfigController", ["$scope", "$location", "$rootScope",
                                           ...
                                           function ($scope, $location,
                                                     ...
                                                     ConfigFactory,instanceUrl,
                                                     inform, gettextCatalog) {
    $scope.defaultSMSProvider = "edumfa.lib.smsprovider.HttpSMSProvider.HttpSMSProvider";
    ...
    $scope.loadSystemConfig = function () {
        ConfigFactory.loadSystemConfig(function (data) {
            ...
            $scope.form['radius.dictfile'] = "/etc/edumfa/dictionary";
            // SMS
